"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createRandomPost = exports.createRandomUser = void 0;
var faker_1 = require("@faker-js/faker");
function createRandomUser() {
    return {
        user_id: faker_1.faker.string.uuid(),
        username: faker_1.faker.internet.userName(),
        first_name: faker_1.faker.person.firstName(),
        last_name: faker_1.faker.person.lastName(),
        age: faker_1.faker.number.int({ min: 18, max: 100 }),
        email: faker_1.faker.internet.email(),
    };
}
exports.createRandomUser = createRandomUser;
function createRandomPost(posterId) {
    return {
        text: faker_1.faker.word.words(),
        posted_by: posterId,
        date: faker_1.faker.date.month(),
    };
}
exports.createRandomPost = createRandomPost;
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var endpoint, numUsers, numPosts, users, posts, i, i, i, response, i, response, result;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    endpoint = "127.0.0.1:8080";
                    numUsers = 100;
                    numPosts = 1000;
                    users = [];
                    posts = [];
                    for (i = 0; i < numUsers; i++) {
                        users.push(createRandomUser());
                    }
                    for (i = 0; i < numPosts; i++) {
                        posts.push(createRandomPost(users[i % numUsers].user_id));
                    }
                    // make api request to endpoint (POST)
                    // send all the users
                    console.log("Sending all the users now");
                    i = 0;
                    _a.label = 1;
                case 1:
                    if (!(i < numUsers)) return [3 /*break*/, 4];
                    return [4 /*yield*/, fetch("".concat(endpoint, "/add_user"), {
                            method: "POST",
                            body: JSON.stringify(users[i]),
                            headers: {
                                "Content-Type": "application/json",
                            },
                        })];
                case 2:
                    response = _a.sent();
                    _a.label = 3;
                case 3:
                    i++;
                    return [3 /*break*/, 1];
                case 4:
                    // send all the posts
                    console.log("Sending all the posts now");
                    i = 0;
                    _a.label = 5;
                case 5:
                    if (!(i < numPosts)) return [3 /*break*/, 8];
                    return [4 /*yield*/, fetch("".concat(endpoint, "/add_post"), {
                            method: "POST",
                            body: JSON.stringify(posts[i]),
                            headers: {
                                "Content-Type": "application/json",
                            },
                        })];
                case 6:
                    response = _a.sent();
                    result = response.json();
                    _a.label = 7;
                case 7:
                    i++;
                    return [3 /*break*/, 5];
                case 8: return [2 /*return*/];
            }
        });
    });
}
main();
